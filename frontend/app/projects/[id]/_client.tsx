...
export default function ProjectDetailPage() {
  ...
  const [expandedUpdateComments, setExpandedUpdateComments] = useState<Set<string>>(new Set());

  const toggleUpdateComments = (updateId: string) => {
    setExpandedUpdateComments((prev) => {
  ...
  // Auto-open update form when navigated with #add-update hash
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hash === '#add-update') {
      setShowUpdateForm(true);
      // Scroll to the updates section after a short delay to allow render
      setTimeout(() => {
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* Likes Section */}
        <div className="mb-8">
          <BubbleButton likeableType="project" likeableId={projectId} />
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-[hsl(var(--surface-container-lowest))] rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold" id="add-update">Progress Updates</h2>
                ...
                <div className="space-y-4">
                  {updates.map((update) => (
                    ...
                    >
                      ...
                      {/* Likes + Comments toggle for update */}
                      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-[hsl(var(--outline-variant))]">
                        <BubbleButton likeableType="project_update" likeableId={update.id} />
                        <button
                          type="button"
                          onClick={() => toggleUpdateComments(update.id)}
                          className="text-sm text-[hsl(var(--on-surface-variant))] hover:text-[hsl(var(--primary))] transition-colors"
                        >
                          {expandedUpdateComments.has(update.id) ? 'Hide comments' : 'Comments'}
                        </button>
                      </div>

                      {/* Collapsible comment section */}
                      {expandedUpdateComments.has(update.id) && (
                        <div className="mt-4">
                          <CommentSection
                            commentableType="project_update"
          ...
          <div className="space-y-6">
            ...
            {project.user && (
              <section className="bg-[hsl(var(--surface-container-lowest))] rounded-lg p-6">
                ...
                >
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-[hsl(var(--surface-container))] flex-shrink-0">
                    {project.user.avatar_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-lg font-semibold text-[hsl(var(--on-surface-variant))]">
                        {(project.user.display_name || project.user.username).charAt(0).toUpperCase()}
                      </div>
                    )}
                  </div>
                  <div>
                    <div className="font-medium group-hover:text-[hsl(var(--primary))] group-hover:underline transition-colors">
