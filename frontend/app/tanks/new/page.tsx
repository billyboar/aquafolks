...
import FishSelector, { type SelectedFish } from '@/components/FishSelector';
import PhotoUploader from '@/components/PhotoUploader';
import type { TankType, CreateTankInput } from '@/lib/types';
...
export default function NewTankPage() {
  ...
  const [formData, setFormData] = useState<CreateTankInput>({
    name: '',
    volume_liters: 0,
    dimensions_length: 0,
    dimensions_width: 0,
  ...
  // Livestock state
  const [livestock, setLivestock] = useState<SelectedFish[]>([]);
  ...
  const handleAddFish = (fish: SelectedFish) => {
    setLivestock([...livestock, fish]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create tank
      const response = await api.post('/api/v1/tanks', formData);
      const tankId = response.data.tank.id;
      
      // Add livestock if any
      if (livestock.length > 0) {
        await Promise.all(
          livestock.map((fish) =>
            api.post(`/api/v1/tanks/${tankId}/livestock`, fish)
          )
        );
      }
      ...
      if (photos.length > 0) {
        await Promise.all(
          photos.map(async (photo, index) => {
            ...
            return api.post(`/api/v1/tanks/${tankId}/photos`, formData, {
              headers: {
      ...
      setError(err.response?.data?.error || err.message || 'Failed to create tank. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-[hsl(var(--on-surface-variant))]">
            Share your aquarium with the community
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="bg-[hsl(var(--surface-container-lowest))] rounded-lg p-6 space-y-6">
            <div>
              <h2 className="text-xl font-semibold mb-2">Livestock</h2>
              <p className="text-sm text-[hsl(var(--on-surface-variant))] mb-4">
                Add fish and invertebrates to your tank (optional)
              </p>
            </div>

            <FishSelector onAdd={handleAddFish} tankType={formData.tank_type} />

            {/* Added livestock list */}
            {livestock.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-sm font-medium">Added ({livestock.length})</h3>
                <div className="space-y-2">
                  {livestock.map((fish, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-3 bg-[hsl(var(--surface))] rounded-lg border border-[hsl(var(--outline-variant))]"
                    >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <button
                          ...
                          className="text-[hsl(var(--error))] hover:text-[hsl(var(--error))] hover:bg-[hsl(var(--error-container))] px-3 py-1 rounded transition-colors"
                        >
                          Remove
                        </button>
                      </div>
          ...
          <section className="bg-[hsl(var(--surface-container-lowest))] rounded-lg p-6 space-y-6">
            <div>
              ...
              <p className="text-sm text-[hsl(var(--on-surface-variant))] mb-4">
                Upload photos of your tank (optional)
              </p>
            </div>
